...
```bash
alias sl="bash ~/tmux-ai/scripts/login.sh"
...
5. 下次 `sl liyufeng_4090` 自动恢复

```bash
# 典型流程
sl liyufeng_4090       # 登录，自动进 tmux
# ...在服务器上启动训练...
# Ctrl-w d                 # 退出，训练继续跑
# 第二天
sl liyufeng_4090       # 自动 attach，查看训练进度
```
...
| [快速登录 & 监控](docs/server-tools.md) | sl 一键登录、server-monitor 多服务器监控面板、健康检查 |
